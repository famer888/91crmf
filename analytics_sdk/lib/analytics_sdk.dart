...
export 'enum/page_route_enum.dart';
...
class AnalyticsSdk {
  ...
  Future<void> init({
    ...
    try {
      ...
      // 注入新 session 回调：新 session 开始时清空广告去重状态
      SessionManager.instance.onNewSession = () => AdImpressionManager.instance.clear();
      SessionManager.instance.initialize();
      // 注入 SID 提供者，避免 entity 层直接依赖 SessionManager
      SdkContext.sidProvider = () {
  ...
  static Map<String, String> getDeviceCommonFields() {
    try {
      return SdkContext.getDeviceCommonFields();
    } catch (e) {
  ...
  static Map<String, dynamic> getParams([List<String>? keys]) {
    try {
      final all = <String, dynamic>{
        'app_id': SdkContext.appId ?? '',
        'channel': SdkContext.channel ?? '',
        'uid': SdkContext.uid ?? '',
        'user_type': UserManager.instance.userType,
        'sid': SessionManager.instance.currentSessionId ?? '',
        'sdk_version': SdkContext.kSdkVersion,
        'app_version': SdkContext.appVersion ?? SdkContext.kDefaultAppVersion,
        ...SdkContext.getDeviceCommonFields(),
      };
  ...
  /// 页面生命周期观察者，需在 [MaterialApp.navigatorObservers] 中注册。
  ///
  /// 注册后 SDK 自动追踪所有命名路由的 [AppPageViewEvent]，无需手动上报。
  ///
  /// ```dart
  /// MaterialApp(
  ///   navigatorObservers: [AnalyticsSdk.instance.pageObserver],
  /// )
  /// ```
  NavigatorObserver get pageObserver => _pageObserver;

  /// 手动上报导航事件，在用户点击导航菜单时调用。
  ///
  /// **适用场景**：底部 Tab、顶部 Tab、侧边抽屉等导航菜单点击。
  /// 内部同时调用 [PageLifecycleObserver.recordNavigation]，更新全局 currentPageKey
  /// 及栈顶 effectivePageKey，确保后续 push 的 referrer 和 page_click 归属均正确。
  ///
  /// **已接入 [BottomNavigationBar.withAnalytics] / [TabBar.withAnalytics] 时无需调用**，
  /// 包装器内部已处理；仅在完全手动管理导航时使用。
  ///
  /// - [pageKey]：导航目标的唯一标识，建议使用 [AppPageEnum]（如 `AppPageEnum.video.key`），
  ///   也可传入自定义字符串；与 app_page_view 的 page_key 保持一致
  /// - [pageName]：导航目标的展示名称，建议使用 [AppPageEnum]（如 `AppPageEnum.video.name`）
  ///
  /// 本方法永远不会抛出异常。
  void trackNavigation({
    ...
    try {
      ...
      track(NavigationEvent(
        navigationKey: pageKey,
        navigationName: pageName,
      ));
    } catch (e) {
      Logger.analyticsSdk('trackNavigation() 异常，已安全处理: $e');
    }
  }

  /// 手动同步当前页面状态，专用于 PageView + AutomaticKeepAliveClientMixin 场景。
  ///
  /// [PageView] 内 Tab 切换不经过路由系统，[PageLifecycleObserver] 无法感知；
  /// 在 `PageView.onPageChanged` 中调用此方法，确保后续 page_click 的 `page_key`
  /// 归属到实际可见的子页面，而非父路由 key。
  ///
  /// 内部调用 [PageLifecycleObserver.recordNavigation]，同时更新
  /// [PageLifecycleObserver.currentPageKey] 和栈顶的 effectivePageKey；
  /// pop 返回时会自动恢复到最后一次调用的 pageKey。
  ///
  /// - [pageKey]：当前可见子页面的唯一标识，建议使用 [AppPageEnum]（如 `AppPageEnum.videoList.key`），
  ///   也可传入自定义字符串
  /// - [pageName]：当前可见子页面的展示名称（仅用于日志，不上报事件）
  ///
  /// 本方法永远不会抛出异常。
  void updateCurrentPage({
    ...
    try {
      PageLifecycleObserver.recordNavigation(pageKey);
      Logger.analyticsSdk('updateCurrentPage: pageKey=$pageKey, pageName=$pageName');
    } catch (e) {
      Logger.analyticsSdk('updateCurrentPage() 异常，已安全处理: $e');
    }
  }
  ...
  void track(dynamic event) {
    try {
      ...
      final adResult = _TrackPipeline.deduplicateAdImpression(event);
      if (adResult == null) return;
      event = adResult.processedEvent;
      try{
        debugLog("track::: ${event.toJson().toString()}");
      }catch(e){
        debugLog("track:::error: ${event.toString()}");
      }
      final json = _TrackPipeline.serializeEvent(event);
      if (json == null) return;

      final validated = _TrackPipeline.validateEvent(json);
      if (validated == null) return;

      if (!_TrackPipeline.checkEventSize(validated)) return;
      if (!_TrackPipeline.isEventTypeEnabled(validated)) return;
  ...
  List<Map<String, String>> getDebugSteps() {
    try {
      ...
      final appIdOk =
          SdkContext.appId != null && SdkContext.appId!.isNotEmpty;
      steps.add({
        'name': '②SDK初始化',
      ...
      if (ep.lastUploadTime == null) {
        ...
        uploadStatus = '✓成功';
        uploadDetail = _DebugInspector.formatTime(ep.lastUploadTime);
      } else {
        uploadStatus = '✗失败';
  ...
  Map<String, dynamic> validateEncryptedConfig(String ciphertext) {
    return _DebugInspector.validateEncryptedConfig(ciphertext);
  }
...
/// track() 管道辅助方法（纯静态，不依赖 AnalyticsSdk 实例）
class _TrackPipeline {
  /// 广告展示事件去重处理。返回 null 表示全部 adId 均已上报，应跳过整条事件。
  static _AdDedupeResult? deduplicateAdImpression(dynamic event) {
    ...
    try {
      ...
      if (unreportedAdIds != null) {
        ...
        if (unreportedAdIds.length < originalAdIdsCount) {
          final filtered = _createFilteredAdEvent(adEvent, unreportedAdIds);
          Logger.analyticsSdk(
  ...
  /// 将事件对象序列化为 JSON Map。返回 null 表示序列化失败，应丢弃事件。
  static Map<String, dynamic>? serializeEvent(dynamic event) {
    try {
  ...
  /// 校验事件字段。返回 null 表示校验不通过，应丢弃事件。
  static Map<String, dynamic>? validateEvent(Map<String, dynamic> json) {
  ...
  /// 检查单条事件大小。返回 false 表示超限应丢弃。
  static bool checkEventSize(Map<String, dynamic> json) {
    try {
      ...
      if (sizeBytes > SdkConfig.maxSingleEventSize) {
        ...
        final limitKB = (SdkConfig.maxSingleEventSize / 1024).toStringAsFixed(0);
        Logger.analyticsSdk(
  ...
  /// 检查事件类型是否在配置中启用。返回 false 表示未启用应跳过。
  static bool isEventTypeEnabled(Map<String, dynamic> json) {
    try {
  ...
  static List<String>? _getUnreportedAdIds(AdImpressionEvent event) {
    try {
  ...
  static AdImpressionEvent _createFilteredAdEvent(
...
/// Debug 调试辅助方法（纯静态，不依赖 AnalyticsSdk 实例）
class _DebugInspector {
  ...
  /// 验证加密配置字符串格式（仅 debug 包可用）
  static Map<String, dynamic> validateEncryptedConfig(String ciphertext) {
    if (kReleaseMode) {
