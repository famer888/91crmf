...
mixin _Black on _BaseAppRepo implements BlackDomain {
  ...
  AsyncResult<BlackDetailModel> getBlackDetail({required int id, String token = ''}) =>
      _blackService.getBlackDetail(id: id, token: token).deserializeJsonBy(BlackDetailModel.fromJson).guard;
  ...
  /// 黑料搜索列表
  @override
  AsyncResult<BlackSearchListModel> searchList({required int page, required int limit, required String word}) => _blackService
      .searchList(page: page, limit: limit, word: word)
      .deserializeJsonBy(BlackSearchListModel.fromJson)
      .guard;
}
