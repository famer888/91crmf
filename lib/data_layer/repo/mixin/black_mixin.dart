...
mixin _Black on _BaseAppRepo implements BlackDomain {
  ...
  AsyncResult<BlackClassModel> getCategoryList({String token = ''})
    => _blackService
      .getCategoryList(token: token)
      .deserializeJsonBy(BlackClassModel.fromJson)
      .guard;
  ...
  AsyncResult<BlackListContentModel> getBlackList({String apiListUrl = '/list_contents', required int mid, required int page, required int limit})
    => _blackService
      .getBlackList(mid: mid, page: page, limit: limit)
      .deserializeJsonBy(BlackListContentModel.fromJson)
      .guard;
  ...
  AsyncResult<BlackPostLikeModel> getBlackLike({required int id})
    => _blackService
    .getBlackLike(id: id)
    .deserializeJsonBy(BlackPostLikeModel.fromJson)
    .guard;
  ...
  AsyncResult<CommentListModel> getBlackCommentList({required int id, required int page, required int limit})
    => _blackService
      .getBlackCommentList(id: id, page: page, limit: limit)
  ...
  AsyncResult publishBlackComment({required int cid, required String content})
    => _blackService
      .publishBlackComment(cid: cid, content: content)
      .deserializeJsonBy((e) => e)
      .guard;
