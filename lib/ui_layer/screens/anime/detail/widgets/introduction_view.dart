...
import 'package:jycrpj/domain/model/cartoon/cartoon_detail_model.dart';
                      ...
                      return GestureDetector(
                        onTap: () async {
                          if (videoInfo.id case final id?) {
                            ...
                            final res = await domain.cartoonFavorite(id: id);
                            if (res.isValid) {
                      ...
                      InkWell(
                        onTap: () async {
                          ...
                          if (member.vipLevel < 1 && false) {
                            ...
                            ///修改数据
                            privilegeDomain
                                .download(id: '${videoInfo.id}')
                                .then((res) {
                              if (res.status == 1) {
                                final downNum = member.cartoonDownValue ?? 0;
                                if (downNum > 0) {
                                  userNotifier.setCartoonDownNum(
                                      num: downNum - 1);
                                }
...
class _RecommendListViewState extends State<RecommendListView> {
  ...
  Widget build(BuildContext context) {
    ...
    return data.isEmpty
        ? Center(
            child: SizedBox(height: 260.w, child: const PageEmptyDataView()))
        : ListView.separated(
