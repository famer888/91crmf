...
class GameMoreScreen extends StatefulWidget {
  const GameMoreScreen({super.key, required this.sort, required this.title});

  final String sort;
  final String title;

  @override
  State<GameMoreScreen> createState() => _GameMoreScreenState();
}

class _GameMoreScreenState extends State<GameMoreScreen> {
  ...
  Future<List<GameModel>?> _getData({
    ...
    final result =
        await _appDomain.gameMore(sort: sort, page: page, limit: pageSize);
