...
class MineCustomerServiceScreen extends StatefulWidget {
  ...
  @override
  State<MineCustomerServiceScreen> createState() => _MineCustomerServiceScreenState();
}
...
class _MineCustomerServiceScreenState extends State<MineCustomerServiceScreen> {
  ...
  Future _imagePickerAssets() async {
    if (await CommonUtils.pickImage() case final xFile?) {
      ...
      if (uploadImageRes != null && uploadImageRes['code'] == 1) {
        ...
        final sendFeedingRes = await _messageDomain.sendFeeding(content: url, type: 2, helpType: 0);
  ...
  Future _sendMsg() async {
    ...
    if (text.isNotEmpty) {
      final result = await _messageDomain.sendFeeding(content: text, type: 1, helpType: 0);
      if (result.isValid) {
        final message = FeedBackData.fromJson({
          'messageType': 1,
          'status': 1,
                ...
                data: (_) => data.isEmpty
                    ...
                    : ListView.builder(
                        ...
                        itemBuilder: (BuildContext context, int index) =>
                            data[index].status == 1 ? _UserBubble(item: data[index]) : _ServiceBubble(item: data[index]),
                      ),
              ),
            ),
            Container(
                    ...
                    children: [
                      ...
                      ReportGestureDetector(
                        ...
                        child: SizedBox(
                          ...
                          height: 44.w,
                          child: Center(
                            child: MyImage.asset(MyImagePaths.appCustomSend, width: 33.w, height: 33.w),
                          ),
                        ),
                      ),
                    ],
...
class _Tips extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Center(child: Text('zxkfts'.tr(context: context), textAlign: TextAlign.center, style: MyTheme.green11)),
    );
  }
}
            ...
            children: [
              ...
              Flexible(
                child: Container(
                  ...
                  padding: EdgeInsets.symmetric(horizontal: 15.5.w, vertical: 14.5.w),
                  child: item.messageType == 1
