...
class _MineWithdrawalScreenState extends State<MineWithdrawalScreen> {
  ...
  Future _initData() async {
    ...
    if (isAgent) {
      final agentWithdraw = (await Future.wait([
        dynamicDomain.getConstructByApiLink(apiLink: 'withdraw/index', params: {}),
        _getDefaultBankCard(),
      ]))[0];
      if (agentWithdraw['status'] == 1) {
        final agentWithdrawData = agentWithdraw['data'];
        final cashWithdrawRule = CashWithdrawRule.fromJson(agentWithdrawData);
        _asyncValue = AsyncData(cashWithdrawRule);
      } else {
        MyToast.showText(text: agentWithdraw.msg ?? '');
        _asyncValue = const AsyncError();
      }
    } else {
      final eventWithdraw = (await Future.wait([
        dynamicDomain.getConstructByApiLink(apiLink: 'eventwithdraw/index', params: {}),
        _getDefaultBankCard(),
      ]))[0];
      if (eventWithdraw['status'] == 1) {
        final eventWithdrawData = eventWithdraw['data'];
        final cashWithdrawRule = CashWithdrawRule.fromJson(eventWithdrawData);
        _asyncValue = AsyncData(cashWithdrawRule);
      } else {
        MyToast.showText(text: eventWithdraw.msg ?? '');
        _asyncValue = const AsyncError();
      }
    }
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Container(
                            ...
                            decoration: BoxDecoration(
                              gradient: const LinearGradient(
                                colors: [
                                  Color.fromRGBO(29, 4, 7, 0.5),
                                  Color.fromRGBO(29, 4, 7, 0.7),
                                  Color.fromRGBO(74, 4, 7, 0.5),
                                ],
                                begin: Alignment.topLeft,
                                end: Alignment.bottomRight,
                              ),
                              borderRadius: BorderRadius.circular(5.w),
                              border: Border.all(color: const Color.fromRGBO(74, 9, 9, 1), width: 0.5.w),
                            ),
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                Container(
                                  alignment: Alignment.centerLeft,
                                  padding: EdgeInsets.only(left: 10.w, right: 10.w, top: 8.w, bottom: 3.w),
                                  ...
                                  child: TextField(
                                    ...
                                    decoration: InputDecoration(
                                      ...
                                      hintStyle: MyTheme.hexa3a2a2_12,
                          ...
                          Row(
                            ...
                            children: [
                              Text('${'xhj'.tr(context: context)}${'kc'.tr(context: context)}: ', style: MyTheme.gray143_13),
                              ValueListenableBuilder(
                                builder: (_, value, __) {
                                  return Text('$value${'y'.tr(context: context)}', style: MyTheme.jellyCyan_13);
                                },
                                valueListenable: _sumAllResultMoney,
                              ),
                              const Text(' '),
                              Text('${'dzhj'.tr(context: context)}: ', style: MyTheme.gray143_13),
                              ValueListenableBuilder(
                                  valueListenable: _sumResultMoney,
                                  builder: (_, value, __) {
                                    return Text('$value', style: MyTheme.jellyCyan_13);
                                  }),
                              Text('y'.tr(context: context), style: MyTheme.jellyCyan_13)
                            ],
                          ),
                          ...
                          GestureDetector(
                            ...
                            child: Container(
                              height: 45.w,
                              padding: EdgeInsets.symmetric(horizontal: MyTheme.pagePadding),
                              decoration: BoxDecoration(borderRadius: BorderRadius.circular(5.w), color: Colors.white.withOpacity(0.2)),
                              child: Row(
                                ...
                                children: [
                                  ValueListenableBuilder(
                                      ...
                                      builder: (_, value, __) {
                                        return value != null
                                            ? Text('${value.bank} ${CommonUtils.subStringFour('${value.card}')}', style: MyTheme.white244_14)
                                            : Text('xztx'.tr(context: context), style: MyTheme.hexa3a2a2_15);
                                      }),
                                  MyImage.asset(MyImagePaths.appYjt, width: 6.w, height: 10.w)
                                ],
                              ),
                            ),
                          ),
                          SizedBox(height: 24.w),
                          Text('txgz'.tr(context: context), style: MyTheme.white255_13_M.s16.w500),
                          SizedBox(height: 5.w),
                          Container(
                            alignment: Alignment.topLeft,
                            child: Text(
                              (isAgent ? data.ruleProxyText : data.ruleCoinsText) ?? data.ruleText ?? '',
                              style: MyTheme.black118_12,
                              strutStyle: StrutStyle(forceStrutHeight: true, height: 1.w, leading: 0.9),
                            ),
                          )
                        ],
                      ),
                      ...
                      child: Offstage(
                        ...
                        child: GestureDetector(
                          ...
                          child: Container(
                            ...
                            child: Text(isAgent ? 'qrtz'.tr(context: context) : 'fqtz'.tr(context: context), style: MyTheme.white234_15_M),
                          ),
                        ),
                      ),
                    ),
