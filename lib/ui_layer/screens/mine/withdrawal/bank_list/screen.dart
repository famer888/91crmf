...
class MineWithdrawalBankListScreen extends StatefulWidget {
  ...
  @override
  State<MineWithdrawalBankListScreen> createState() => _MineWithdrawalBankListScreenState();
}

class _MineWithdrawalBankListScreenState extends State<MineWithdrawalBankListScreen> {
  ...
  void _showAddBankCardDialog() {
    ...
    Widget buildTextField(String hint, TextEditingController controller, TextInputType type) {
      return TextField(
        textAlign: TextAlign.left,
        controller: controller,
        style: MyTheme.white15bold.s14,
        cursorColor: Colors.white,
        keyboardType: type,
        decoration: InputDecoration(
          contentPadding: EdgeInsets.symmetric(horizontal: MyTheme.pagePadding, vertical: MyTheme.pagePadding),
          hintText: hint.tr(context: context),
          hintStyle: MyTheme.white255_13_M.s12.w400,
          focusedBorder: OutlineInputBorder(
            borderRadius: BorderRadius.circular(4.w),
            borderSide: const BorderSide(color: Color.fromRGBO(35, 30, 32, 1)),
          ),
              ...
              children: [
                SingleChildScrollView(
                  child: Column(
                    ...
                    children: [
                      Container(alignment: Alignment.center, child: Text('tjzh'.tr(context: context), style: MyTheme.white244_20_M)),
                      SizedBox(height: 20.w),
                      Row(
                        mainAxisAlignment: MainAxisAlignment.spaceBetween,
                        children: [
                          Text('zhlx'.tr(context: context), style: MyTheme.white255_13_M.w400),
                          Text('yhk'.tr(context: context), style: MyTheme.white255_13_M.w400)
                        ],
                      ),
                      SizedBox(height: 30.w),
                      Column(children: [
                        // 银行名称
                        buildTextField('sryh', cardController, TextInputType.text),
                        SizedBox(height: 10.w),
                        // 银行卡号
                        buildTextField('srkh', numController, TextInputType.number),
                        SizedBox(height: 10.w),
                        // 持卡人
                      ...
                      GestureDetector(
                        ...
                        child: Container(
                          ...
                          child: Center(child: Text('qr'.tr(context: context), style: MyTheme.white255_15)),
                        ),
                      )
                    ],
                  ),
  ...
  Future<void> _sendAddBankCard({
    ...
    try {
      if (card.isNotEmpty && name.isNotEmpty) {
        MyToast.showLoading();
        final result = await userDomain.cashAddBankCard(card: card, name: name, num: num);
        if (result.isValid) {
        ...
        body: Stack(
          children: [
            MyListView.list(
              ...
              onFetchingMore: (currentPage, pageSize) => getBankCardList(currentPage: currentPage, limit: pageSize),
            ),
            SafeArea(
              child: Align(
