...
class _VoiceAIRecordCardState extends State<VoiceAIRecordCard> {
  ...
  Future<void> _initController() async {
    if (_audioUrl.isEmpty || _controller != null || _isInitializing) {
    ...
    final controller = VideoPlayerController.networkUrl(Uri.parse(_audioUrl));
    try {
      await controller.initialize();
      controller.addListener(_onControllerValueChanged);
      if (!mounted) {
        controller
          ..removeListener(_onControllerValueChanged)
          ..dispose();
        return;
      }
      setState(() {
  ...
  Future<void> _togglePlay() async {
    if (!_isInit || _controller == null) {
      MyToast.showText(
          text: _isInitializing ? '播放器加载中,请稍候...' : '音频不可用，请稍后再试');
      return;
    }
    if (_isPlaying) {
  ...
  void _onControllerValueChanged() {
    ...
    final shouldReset = value.isCompleted ||
        (value.position >= value.duration &&
            value.duration.inMilliseconds > 0);
    setState(() {
      _progress = shouldReset ? Duration.zero : value.position;
  ...
  Widget build(BuildContext context) {
    return Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
      ...
      Container(
        height: 44.w,
        decoration: BoxDecoration(
            color: const Color.fromRGBO(35, 30, 32, 1),
        ...
        child: Row(children: [
          ReportGestureDetector(
            ...
            child: Icon(_isPlaying ? Icons.pause : Icons.play_arrow,
                color: MyTheme.blueColor64, size: 24.w),
          ),
          SizedBox(width: 4.w),
