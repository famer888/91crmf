...
class MineShareToUserRecordScreen extends StatefulWidget {
  ...
  @override
  State<MineShareToUserRecordScreen> createState() => _MineShareToUserRecordScreenState();
}

class _MineShareToUserRecordScreenState extends State<MineShareToUserRecordScreen> {
  late final _domain = context.read<ProxyDomain>();
  late final _inviteDomain = context.read<InviteDomain>();
  ...
  Future<void> _getInviteData({required int currentPage, required int limit}) async {
    final result = await _inviteDomain.getInviteRecord(page: currentPage, limit: limit);
    CommonUtils.log('获取邀请记录列表的结果:$result');
  }

  @override
  void initState() {
    super.initState();
    _getInviteData(currentPage: 1, limit: 10);
  }
