...
// import 'package:jycrpj/ui_layer/screens/common_widgets/auto_carousel_slider.dart';
...
class NewGeneralAppListSwiper extends StatefulWidget {
  NewGeneralAppListSwiper({
    super.key,
    required this.data,
    this.radius = 5,
  ...
  @override
  State<NewGeneralAppListSwiper> createState() => _NewGeneralAppListSwiperState();
}

class _NewGeneralAppListSwiperState extends State<NewGeneralAppListSwiper> {
    ...
    if (widget.data.length > threshold) {
      ...
      return Column(
        children: [
          ...
          if (secondPart.isNotEmpty && secondPart is List<BannerModel>)
            InfiniteBannerList(banners: secondPart, columNumber: _ColumNumber, titleColor: widget.titleColor),
        ],
      );
    } else {
...
class InfiniteBannerList extends StatefulWidget {
  ...
  const InfiniteBannerList(
      {required this.banners, required this.columNumber, this.titleColor, super.key});
