...
import 'package:jycrpj/ui_layer/screens/crack/apps/zpc/widget/zpc_collect_button.dart';
...
class ZpcVideoDetailScreen extends StatefulWidget {
  final int id;

  const ZpcVideoDetailScreen({super.key, required this.id});

  @override
  State<ZpcVideoDetailScreen> createState() => _ZpcVideoDetailScreenState();
}

class _ZpcVideoDetailScreenState extends State<ZpcVideoDetailScreen> {
  ...
  Future _initData() async {
    ...
    final result = await _appDomain.getConstructByApiLink(apiLink: 'mvzpc/getDetail', params: {'id': widget.id});
    if (result.status == 1) {
  ...
  void _getVideoRecommendList() async {
    final result = await _appDomain.getConstructByApiLink(apiLink: 'mvzpc/getDetailRecommendList', params: {'id': widget.id});
    if (result.status == 1) {
  ...
  void _videoDownload(VideoData videoData) async {
    ...
    final result = await _appDomain.getConstructByApiLink(apiLink: 'mvzpc/download', params: {'id': widget.id});
  ...
  Future<void> _startDownload(String downloadUrl, VideoData videoData) async {
    ...
    privilegeDomain.downNum(id: '${videoData.id}', type: CrackAppType.zpc.type).then((res) {
      if (res.status == 1) {
                    ...
                    data: (data) => Column(
                      children: [
                        ...
                        Container(
                          ...
                          child: Row(
                            children: [
                              Text('简介', style: TextStyle(color: MyTheme.blackColor18, fontSize: 16.sp, fontWeight: FontWeight.w500)),
                              const Spacer(),
...
class _BodyState extends State<_Body> with TickerProviderStateMixin {
              ...
              children: [
                ...
                Expanded(
                  child: Text(
                    ...
                    style: TextStyle(color: MyTheme.blackColor32, fontSize: 14.sp, fontWeight: FontWeight.w400),
                    maxLines: 2,
                    softWrap: true,
                  ),
                ),
              ...
              children: [
                ...
                Expanded(
                    child: Text('xgtj'.tr(context: context),
                        style: TextStyle(color: MyTheme.blackColor32, fontSize: 12.sp, fontWeight: FontWeight.w500))),
                SizedBox(width: 13.w),
              ],
            ),
            SizedBox(height: 10.w),
                  ...
                  return Padding(
                    ...
                    child: GridView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return ZpcFeedCard(isList: false, feed: item, isInVideoDetail: true);
                      },
                    ),
                  );
                }),
  ...
  Widget _buildBannerWidget(List<Notice>? list) {
    ...
    try {
      ...
      return Padding(
          padding: EdgeInsets.all(MyTheme.pagePadding),
          child: ReportGeneralAppsListVidget(data: banners, titleColor: const Color.fromRGBO(51, 51, 51, 1)));
    } catch (e) {
  ...
  Widget _buildStatisticalDataWidget() {
    Widget current = Row(children: [
      Expanded(
        child: Text(
          '${CommonUtils.formatNumber(widget.data.detail.countPlay ?? 0)}次观看',
          style: TextStyle(
          ...
          child: Row(
            ...
            children: [
              SizedBox(
                width: 20.w,
                height: 20.w,
                child: MyImage.asset(MyImagePaths.appDownloadIcon, width: 20.w, height: 20.w, color: const Color.fromRGBO(51, 51, 51, 1)),
              ),
              SizedBox(width: 5.w),
              Text('xz'.tr(context: context), style: TextStyle(color: MyTheme.blackColor32, fontSize: 13.sp, fontWeight: FontWeight.w400))
            ],
          ),
        ),
      ),
        ...
        child: Row(
          ...
          children: [
            SizedBox(
              width: 20.w,
              height: 20.w,
              child: MyImage.asset(MyImagePaths.appShareIcon, width: 20.w, height: 20.w, color: const Color.fromRGBO(51, 51, 51, 1)),
            ),
            SizedBox(width: 5.w),
            Text('fx'.tr(context: context), style: TextStyle(color: MyTheme.blackColor32, fontSize: 13.sp, fontWeight: FontWeight.w400))
          ],
        ),
      ),
    ]);
    ...
    Widget current = Wrap(
      ...
      children: tags.map((e) {
        return Convenience.buildChildActionWidget(
          onTap: () {
            ZpcVideoTagRoute(e).push(context);
          },
          padding: EdgeInsets.fromLTRB(8.w, 2.w, 8.w, 2.w),
          borderRadius: BorderRadius.circular(2.w),
          gradient: const LinearGradient(colors: [Color.fromRGBO(90, 90, 90, 1), Color.fromRGBO(90, 90, 90, 1)]),
