...
import 'package:jycrpj/ui_layer/screens/crack/apps/zpc/widget/zpc_sub_list_page.dart';
import 'package:jycrpj/ui_layer/screens/crack/apps/zpc/widget/zpc_subpage_tab_bar.dart';
...
class ZpcHomeScreen extends StatefulWidget {
  ...
  @override
  State<ZpcHomeScreen> createState() => _ZpcHomeScreenState();
}

class _ZpcHomeScreenState extends State<ZpcHomeScreen> {
    ...
    if (data case final List data when data.isNotEmpty) {
      ...
      _topTabBarDataNotifier.value = linkModelList
          .map((e) => CustomTabItem(
                ...
                activeTextColor: MyTheme.zpcAppPrimaryColor,
                inactiveTextColor: MyTheme.defaultAppTabTitleColor,
                indicatorGradient: const LinearGradient(colors: [MyTheme.zpcAppPrimaryColor, MyTheme.zpcAppPrimaryColor]),
              ))
          .toList();
  ...
  Future<void> _initialRefreshSubPageData() async {
    ...
    if (_currentLinkModel == null) {
      ...
      final index = (_homeConfig.config.zpcSortNav ?? []).indexWhere((item) => item.type == 'new');
      if (index == -1) {
        // 不存在 new
        initialIndex = 0;
      } else {
    ...
    List<AppNavModel> appNavList = _homeConfig.config.zpcSortNav ?? [];
    ...
    return SliverPersistentHeader(
      ...
      delegate: StickyHeaderDelegate(
        ...
        child: StickySearchTabBar(
          ...
          searchAppBarType: CrackAppType.zpc,
          backgroundColor: MyTheme.zpcAppBgColor,
          sideColor: MyTheme.zpcAppSearchBarBackgroundColor,
          searchAppBackgroundColor: MyTheme.zpcAppSearchBarBackgroundColor,
  ...
  Widget _buildScrollSliver() {
    return SliverToBoxAdapter(
      child: BannerTopicsView(
        ...
        topicBackgroundColor: const Color.fromRGBO(90, 90, 90, 1),
        zkTextColor: const Color.fromRGBO(100, 100, 100, 1),
        moreVideoApi: 'mvzpc/list_construct',
          ...
          return Stack(
            children: [
              SmartRefresher(
                ...
                enablePullDown: isUnlockZpc91,
                enablePullUp: isUnlockZpc91,
