...
import 'package:jycrpj/ui_layer/screens/crack/apps/zpc/widget/zpc_api_link_view.dart';
...
class ZpcTopNaviView extends StatefulWidget {
  const ZpcTopNaviView({super.key, this.id, this.crackApp});

  final int? id;
  final CrackApp? crackApp;

  @override
  State<ZpcTopNaviView> createState() => _ZpcTopNaviViewState();
}

class _ZpcTopNaviViewState extends State<ZpcTopNaviView> with TickerProviderStateMixin {
  ...
  Future<void> _init() async {
    ...
    final result = await _appDomain.getConstructByApiLink(apiLink: CrackAppType.zpc.topNavApi, params: {'id': widget.id});
    ...
    return Selector<UnlockStatusNotifier, bool>(
        ...
        builder: (context, isUnlockZpc91, child) {
          return Stack(children: [
            _asyncValue.maybeWhen(
              data: (data) {
                ...
                return TabBarWithView.line(
                  ...
                  linearColors: const [MyTheme.zpcAppPrimaryColor, MyTheme.zpcAppPrimaryColor],
                  labelStyle: TextStyle(color: MyTheme.zpcAppPrimaryColor, fontSize: 18.sp, fontWeight: FontWeight.w600),
                  unselectedLabelStyle: TextStyle(color: const Color.fromRGBO(97, 97, 101, 1), fontSize: 16.sp, fontWeight: FontWeight.w500),
                  titles: titles,
                  views: data.map((e) {
                    return KeepAliveWrapper(
                      child: ZpcApiLinkView(
                        linkModel: e,
                        showRightList: true,
                        onLinkNavTap: (value) {
            ...
            /// ===== 蒙层（不穿透）=====
            if (!isUnlockZpc91 && widget.crackApp != null)
              LockMask(
