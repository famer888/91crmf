...
class ZpcSubListPage extends StatefulWidget {
  ...
  @override
  State<ZpcSubListPage> createState() => _ZpcSubListPageState();
}

class _ZpcSubListPageState extends State<ZpcSubListPage> {
  ...
  SliverGrid _buildSliverGrid() {
    return SliverGrid(
      ...
      delegate: SliverChildBuilderDelegate(
        (context, index) {
          final feed = widget.dataListNotifier.value[index];
          return ZpcFeedCard(isList: false, feed: feed);
        },
        childCount: widget.dataListNotifier.value.length,
      ),
    );
