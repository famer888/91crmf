...
import 'package:jycrpj/ui_layer/screens/crack/apps/awjq/widget/awjq_sub_list_page.dart';
import 'package:jycrpj/ui_layer/screens/crack/apps/awjq/widget/awjq_subpage_tab_bar.dart';
...
class AwjqHomeScreen extends StatefulWidget {
  ...
  @override
  State<AwjqHomeScreen> createState() => _AwjqHomeScreenState();
}

class _AwjqHomeScreenState extends State<AwjqHomeScreen> {
    ...
    if (data case final List data when data.isNotEmpty) {
      ...
      _topTabBarDataNotifier.value = linkModelList
          .map((e) => CustomTabItem(
                ...
                activeTextColor: MyTheme.awjqAppPrimaryColor,
                inactiveTextColor: MyTheme.whiteColor,
                indicatorGradient: const LinearGradient(colors: [MyTheme.awjqAppPrimaryColor, MyTheme.awjqAppPrimaryColor]),
              ))
          .toList();
  ...
  Future<void> _initialRefreshSubPageData() async {
    ...
    if (_currentLinkModel == null) {
      ...
      final index = (_homeConfig.config.awjqSortNav ?? []).indexWhere((item) => item.type == 'new');
      if (index == -1) {
        // 不存在 new
        initialIndex = 0;
      } else {
    ...
    List<AppNavModel> appNavList = _homeConfig.config.awjqSortNav ?? [];
    ...
    return SliverPersistentHeader(
      ...
      delegate: StickyHeaderDelegate(
        ...
        child: StickySearchTabBar(
          ...
          searchAppBarType: CrackAppType.awjq,
          backgroundColor: MyTheme.awjqAppBgColor,
          searchAppBackgroundColor: MyTheme.awjqAppSearchBarBackgroundColor,
  ...
  Widget _buildScrollSliver() {
    return SliverToBoxAdapter(
      child: BannerTopicsView(
        ...
        moreVideoApi: 'mvawjq/list_construct',
  ...
  Widget build(BuildContext context) {
    return Selector<UnlockStatusNotifier, bool>(
        ...
        builder: (context, isUnlockAwjq, child) {
          return Stack(
            children: [
              SmartRefresher(
                controller: _refreshController,
                enablePullDown: isUnlockAwjq,
                enablePullUp: isUnlockAwjq,
