...
import 'package:jycrpj/ui_layer/screens/crack/apps/awjq/widget/awjq_api_link_view.dart';
...
class AwjqTopNaviView extends StatefulWidget {
  const AwjqTopNaviView({super.key, this.id, this.crackApp});

  final int? id;
  final CrackApp? crackApp;

  @override
  State<AwjqTopNaviView> createState() => _AwjqTopNaviViewState();
}

class _AwjqTopNaviViewState extends State<AwjqTopNaviView> with TickerProviderStateMixin {
  ...
  Future<void> _init() async {
    ...
    final result = await _appDomain.getConstructByApiLink(apiLink: CrackAppType.awjq.topNavApi, params: {'id': widget.id});
    ...
    return Selector<UnlockStatusNotifier, bool>(
        ...
        builder: (context, isUnlockAwjq, child) {
          return Stack(children: [
            _asyncValue.maybeWhen(
              data: (data) {
                ...
                return TabBarWithView.line(
                  ...
                  linearColors: const [MyTheme.awjqAppPrimaryColor, MyTheme.awjqAppPrimaryColor],
                  labelStyle: TextStyle(color: MyTheme.awjqAppPrimaryColor, fontSize: 18.sp, fontWeight: FontWeight.w600),
                  unselectedLabelStyle: TextStyle(color: const Color.fromRGBO(255, 255, 255, 1), fontSize: 16.sp, fontWeight: FontWeight.w500),
                  titles: titles,
                  views: data.map((e) {
                    return KeepAliveWrapper(
                      child: AwjqApiLinkView(
                        linkModel: e,
                        showRightList: true,
                        onLinkNavTap: (value) {
            ...
            /// ===== 蒙层（不穿透）=====
            if (!isUnlockAwjq && widget.crackApp != null)
              LockMask(
