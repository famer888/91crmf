...
class AwjqCollectButton extends StatefulWidget {
  final int id;
  final String apiUrl;
  final bool isCollected;
  final CollectCallback callback;

  const AwjqCollectButton({super.key, required this.apiUrl, required this.isCollected, required this.id, required this.callback});

  @override
  State<AwjqCollectButton> createState() => _AwjqCollectButtonState();
}

class _AwjqCollectButtonState extends State<AwjqCollectButton> {
  ...
  Future<void> onCollect() async {
    final result = await _appDomain.getConstructByApiLink(
      ...
      params: {'type': '${CrackAppType.awjq.type}', 'relatedId': widget.id},
    );
    if (result.status == 1) {
      onChangeCollected();
    } else {
      ...
      child: Row(
        ...
        children: [
          SizedBox(
            ...
            height: 20.w,
            child: _isCollected
                ? MyImage.asset(MyImagePaths.appCollectRedOn, width: 20.w, height: 20.w, color: MyTheme.awjqAppPrimaryColor)
