...
import 'package:jycrpj/ui_layer/screens/crack/apps/91aw/widget/aw91_sub_list_page.dart';
import 'package:jycrpj/ui_layer/screens/crack/apps/91aw/widget/aw91_subpage_tab_bar.dart';
...
class Aw91HomeScreen extends StatefulWidget {
  ...
  @override
  State<Aw91HomeScreen> createState() => _Aw91HomeScreenState();
}

class _Aw91HomeScreenState extends State<Aw91HomeScreen> {
    ...
    if (data case final List data when data.isNotEmpty) {
      ...
      _topTabBarDataNotifier.value = linkModelList
          .map((e) => CustomTabItem(
                ...
                indicatorGradient: const LinearGradient(colors: [MyTheme.aw91AppPrimaryColor, MyTheme.aw91AppPrimaryColor]),
              ))
          .toList();
  ...
  Future<void> _initialRefreshSubPageData() async {
    ...
    if (_currentLinkModel == null) {
      ...
      final index = (_homeConfig.config.aw91SortNav ?? []).indexWhere((item) => item.type == 'new');
      if (index == -1) {
        // 不存在 new
        initialIndex = 0;
      } else {
    ...
    List<AppNavModel> appNavList = _homeConfig.config.aw91SortNav ?? [];
  ...
  Future<List<FeedModel>?> _getData({
    ...
    if (result.status == 1) {
      if (result.data['banner'] case final List data when data.isNotEmpty && refresh) {
        CommonUtils.log('aw91 的banner数据');
    ...
    return SliverPersistentHeader(
      ...
      delegate: StickyHeaderDelegate(
        ...
        child: StickySearchTabBar(
          ...
          searchAppBarType: CrackAppType.aw91,
  ...
  Widget _buildScrollSliver() {
    return SliverToBoxAdapter(
      child: BannerTopicsView(
        ...
        moreVideoApi: 'mv91aw/list_construct',
          ...
          return Stack(
            children: [
              SmartRefresher(
                ...
                enablePullDown: isUnlockAw91,
                enablePullUp: isUnlockAw91,
