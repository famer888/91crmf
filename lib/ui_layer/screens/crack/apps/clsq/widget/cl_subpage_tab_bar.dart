...
class ClSubPageTabBar extends StatefulWidget {
  final ValueNotifier<bool> isListNotifier;
  /// second tab bar的选中位置
  final ValueNotifier<int> subPageBarIndexNotifier;
  final bool isYearsOld17;
  final Function(AppNavModel)? onChangeTab;

  const ClSubPageTabBar({
    super.key,
    required this.isListNotifier,
    required this.subPageBarIndexNotifier,
    this.isYearsOld17 = false,
    this.onChangeTab,
  });

  @override
  State<ClSubPageTabBar> createState() => _ClSubPageTabBarState();
}

class _ClSubPageTabBarState extends State<ClSubPageTabBar> {

  late final _homeConfig = context.read<HomeConfigNotifier>();

  List<AppNavModel> _getAppNavList() {
    return widget.isYearsOld17 ? (_homeConfig.config.hjgjDiscoverSortNav ?? []) : (_homeConfig.config.hjgjSortNav ?? []);
  }
