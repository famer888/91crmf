...
import 'package:jycrpj/ui_layer/screens/crack/apps/clsq/widget/cl_sub_list_page.dart';
import 'package:jycrpj/ui_layer/screens/crack/apps/clsq/widget/cl_subpage_tab_bar.dart';
...
import 'package:jycrpj/ui_layer/screens/crack/widgets/banner_topics_view.dart';
...
class _ClHomeScreenState extends State<ClHomeScreen> with TickerProviderStateMixin {
  ...
  /// 列表数据
  final ValueNotifier<List<FeedModel>> _dataListNotifier = ValueNotifier([]);

  /// 轮播数据
  final ValueNotifier<List<BannerModel>> _bannersNotifier = ValueNotifier([]);

  /// 话题数据
  final ValueNotifier<List<NavModel>> _topicsNotifier = ValueNotifier([]);
  ...
  bool get _isYearsOld17 => _currentLinkModel == null ? false : _currentLinkModel!.isNavPrepend;
    ...
    try {
      if (result.status == 1) {
        ...
        if (data case final List data when data.isNotEmpty) {
          ...
          if ((_homeConfig.config.hjgjDiscoverSortNav ?? []).isNotEmpty) {
      ...
      if ((_homeConfig.config.hjgjDiscoverSortNav ?? []).isNotEmpty) {
        ...
        _topTabBarDataNotifier.value = linkModelList
            .map((e) => CustomTabItem(
                  ...
                  activeTextColor: MyTheme.clAppPrimaryColor,
                  inactiveTextColor: MyTheme.whiteColor,
                  indicatorGradient: const LinearGradient(colors: [MyTheme.clAppPrimaryColor, MyTheme.clAppPrimaryColor]),
                ))
            .toList();
      } else {
        _linkModelList = linkModelList;
  ...
  Future<void> _initialRefreshSubPageData() async {
    ...
    if (_currentLinkModel == null) {
      ...
      if (_isYearsOld17) {
        final index = (_homeConfig.config.hjgjDiscoverSortNav ?? []).indexWhere((item) => item.type == 'new');
        if (index == -1) {
          // 不存在 rank
          initialIndex = 0;
        } else {
        ...
        final index = (_homeConfig.config.hjgjSortNav ?? []).indexWhere((item) => item.type == 'new');
        if (index == -1) {
          // 不存在 new
          initialIndex = 0;
        } else {
    ...
    List<AppNavModel> appNavList = _isYearsOld17 ? (_homeConfig.config.hjgjDiscoverSortNav ?? []) : (_homeConfig.config.hjgjSortNav ?? []);
  ...
  Future<void> _loadData({bool refresh = false}) async {
    ...
    try {
      ...
      if (data != null) {
        if (refresh) {
          final list = List<FeedModel>.of(data);
          CommonUtils.log('刷新的数据 :${list.hashCode}');
          _dataListNotifier.value = list;
          _currentPage = 2;
        } else {
          final list = List<FeedModel>.of(_dataListNotifier.value)..addAll(data);
  ...
  Future<List<FeedModel>?> _getData({
    ...
    if (result.status == 1) {
      ...
      if (result.data['nav'] case final List data when data.isNotEmpty && refresh) {
        _topicsNotifier.value = data.map((e) => NavModel.fromJson(e)).toList();
      }

      if (result.data['list'] case final List data when data.isNotEmpty) {
        return data.map<FeedModel>((e) => FeedModel.fromJson(e)).toList();
      }
    } else {
    ...
    return SliverPersistentHeader(
      ...
      delegate: StickyHeaderDelegate(
        ...
        child: StickySearchTabBar(
          ...
          searchAppBarType: CrackAppType.clsq,
          backgroundColor: MyTheme.clAppBgColor,
          searchAppBackgroundColor: MyTheme.defaultSearchBarBackgroundColor,
  ...
  Widget _buildScrollSliver() {
    return SliverToBoxAdapter(
      child: BannerTopicsView(
        bannersNotifier: _bannersNotifier,
        topicsNotifier: _topicsNotifier,
        titleColor: MyTheme.whiteColor,
        topicBackgroundColor: MyTheme.pzhanAppSearchBarBackgroundColor,
        zkTextColor: MyTheme.white07Color,
        moreVideoApi: 'mvhjgj/list_construct',
          ...
          return Stack(
            children: [
              ...
              SmartRefresher(
                ...
                enablePullDown: isUnlockClsq,
                enablePullUp: isUnlockClsq,
