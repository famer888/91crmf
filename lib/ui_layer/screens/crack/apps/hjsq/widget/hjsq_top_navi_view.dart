...
class HjsqTopNaviView extends StatefulWidget {
  const HjsqTopNaviView({super.key, this.id, this.crackApp});

  final int? id;
  final CrackApp? crackApp;

  @override
  State<HjsqTopNaviView> createState() => _HjsqTopNaviViewState();
}

class _HjsqTopNaviViewState extends State<HjsqTopNaviView> with TickerProviderStateMixin {
  ...
  Future<void> _init() async {
    ...
    final result = await _appDomain.getConstructByApiLink(apiLink: CrackAppType.hjsq.topNavApi, params: {'id': widget.id});

    if (result.status == 1) {
      final data = result['data'];
      if (data['value'] case final List data when data.isNotEmpty) {
    ...
    return Selector<UnlockStatusNotifier, bool>(
        ...
        builder: (context, isUnlockHjsq, child) {
          return Stack(children: [
                ...
                return LayoutBuilder(builder: (context, constraints) {
                  return SizedBox(
                    ...
                    child: TabBarWithView.line(
                      ...
                      tabBarHeight: 40.h,
                      linearColors: const [MyTheme.hjsqAppPrimaryColor, MyTheme.hjsqAppPrimaryColor],
                      labelStyle: TextStyle(color: MyTheme.hjsqAppPrimaryColor, fontSize: 18.sp, fontWeight: FontWeight.w600),
            ...
            /// ===== 蒙层（不穿透）=====
            if (!isUnlockHjsq && widget.crackApp != null)
              LockMask(
