...
import 'package:jycrpj/ui_layer/screens/crack/apps/hjsq/widget/hjsq_collect_button.dart';
...
class HjsqVideoDetailScreen extends StatefulWidget {
  final int id;

  const HjsqVideoDetailScreen({super.key, required this.id});

  @override
  State<HjsqVideoDetailScreen> createState() => _HjsqVideoDetailScreenState();
}

class _HjsqVideoDetailScreenState extends State<HjsqVideoDetailScreen> {
  ...
  Future _initData() async {
    ...
    final result = await _appDomain.getConstructByApiLink(apiLink: 'mvhjsq/getDetail', params: {'id': widget.id});
    if (result.status == 1) {
  ...
  void _getVideoRecommendList() async {
    final result = await _appDomain.getConstructByApiLink(apiLink: 'mvhjsq/getDetailRecommendList', params: {'id': widget.id});
    if (result.status == 1) {
  ...
  void _videoDownload(VideoData videoData) async {
    ...
    final result = await _appDomain.getConstructByApiLink(apiLink: 'mvhjsq/download', params: {'id': widget.id});
  ...
  Future<void> _startDownload(String downloadUrl, VideoData videoData) async {
    ...
    privilegeDomain.downNum(id: '${videoData.id}', type: CrackAppType.hjsq.type).then((res) {
      if (res.status == 1) {
                        ...
                        return Padding(
                          ...
                          child: GridView.builder(
                            ...
                            itemBuilder: (context, index) {
                              ...
                              return HjsqFeedCard(isList: false, feed: item, isInVideoDetail: true);
                            },
                          ),
                        );
                      }),
    ...
    Widget current = Wrap(
      ...
      children: tags.map((e) {
        return Convenience.buildChildActionWidget(
          onTap: () {
            HjsqVideoTagRoute(e).push(context);
          },
