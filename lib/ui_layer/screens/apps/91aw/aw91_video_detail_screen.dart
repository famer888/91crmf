...
import 'package:jycrpj/ui_layer/screens/apps/91aw/widget/aw91_collect_button.dart';
...
class Aw91VideoDetailScreen extends StatefulWidget {
  ...
  const Aw91VideoDetailScreen({super.key, required this.id});

  @override
  State<Aw91VideoDetailScreen> createState() => _Aw91VideoDetailScreenState();
}

class _Aw91VideoDetailScreenState extends State<Aw91VideoDetailScreen> {
  ...
  Future _initData() async {
    ...
    final result = await _appDomain.getConstructByApiLink(apiLink: 'mv91aw/getDetail', params: {'id': widget.id});
    if (result.status == 1) {
  ...
  void _getVideoRecommendList() async {
    final result = await _appDomain.getConstructByApiLink(apiLink: 'mv91aw/getDetailRecommendList', params: {'id': widget.id});
    if (result.status == 1) {
  ...
  void _videoDownload(VideoData videoData) async {
    ...
    final result = await _appDomain.getConstructByApiLink(apiLink: 'mv91aw/download', params: {'id': widget.id});
  ...
  Future<void> _startDownload(String downloadUrl, VideoData videoData) async {
    ...
    privilegeDomain.downNum(id: '${videoData.id}', type: CrackAppType.aw91.type).then((res) {
      if (res.status == 1) {
    ...
    Widget current = Wrap(
      ...
      children: tags.map((e) {
        return Convenience.buildChildActionWidget(
          onTap: () {
            Aw91TagRoute(videoTag: e).push(context);
          },
