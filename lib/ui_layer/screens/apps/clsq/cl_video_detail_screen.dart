...
import '../../theme.dart';
import 'widget/cl_collect_button.dart';

class ClVideoDetailScreen extends StatefulWidget {
  final int id;

  const ClVideoDetailScreen({super.key, required this.id});

  @override
  State<ClVideoDetailScreen> createState() => _ClVideoDetailScreenState();
}

class _ClVideoDetailScreenState extends State<ClVideoDetailScreen> {
  ...
  Future _initData() async {
    ...
    final result = await _appDomain.getConstructByApiLink(apiLink: 'mvhjgj/getDetail', params: {'id': widget.id});
    if (result.status == 1) {
  ...
  void _getVideoRecommendList() async {
    final result = await _appDomain.getConstructByApiLink(apiLink: 'mvhjgj/getDetailRecommendList', params: {'id': widget.id});
    if (result.status == 1) {
  ...
  void _videoDownload(VideoData videoData) async {
    ...
    final result = await _appDomain.getConstructByApiLink(apiLink: 'mvhjgj/download', params: {'id': widget.id});
                  ...
                  return Padding(
                    ...
                    child: GridView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return ClFeedCard(isList: false, feed: item, isInVideoDetail: true);
                      },
                    ),
                  );
                }),
    ...
    Widget current = Wrap(
      ...
      children: tags.map((e) {
        return Convenience.buildChildActionWidget(
          onTap: () {
            ClVideoTagRoute(e).push(context);
          },
